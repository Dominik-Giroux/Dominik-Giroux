  ...
  return (
    <>
      <header className="animate-slideTop bg-transparent text-xl font-black">
        <nav className="mx-auto flex max-w-7xl flex-col justify-between gap-4 px-4 py-6 sm:px-6 lg:flex-row xl:gap-6">
          <Link href="/">
            ...
            {menu.links.map((link, i) => (
              <li key={`link-${i}`}>
                <Link href={link.url[context.locale] ? link.url[context.locale] : "/"}>
                  <a className={`nav-link ${`/${link.url[context.locale]}` == router.asPath ? "underline" : null}`}>
